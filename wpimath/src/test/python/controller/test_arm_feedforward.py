...
def test_achievable_velocity():
    ...
    assert ff.max_achievable_velocity(12.0, math.pi / 3, 1.0) == pytest.approx(6.0, abs=0.002)
    assert ff.min_achievable_velocity(11.5, math.pi / 3, 1.0) == pytest.approx(-9.0, abs=0.002)


def test_achievable_acceleration():
    ff = ArmFeedforward(KS, KG, KV, KA)

    assert ff.max_achievable_acceleration(12.0, math.pi / 3, 1.0) == pytest.approx(4.75, abs=0.002)
    assert ff.max_achievable_acceleration(12.0, math.pi / 3, -1.0) == pytest.approx(6.75, abs=0.002)
    assert ff.min_achievable_acceleration(12.0, math.pi / 3, 1.0) == pytest.approx(-7.25, abs=0.002)
    assert ff.min_achievable_acceleration(12.0, math.pi / 3, -1.0) == pytest.approx(-5.25, abs=0.002)
