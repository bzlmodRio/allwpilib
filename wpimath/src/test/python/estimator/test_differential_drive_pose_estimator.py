...
def _test_follow_trajectory(
    ...
    estimator.reset_position(Rotation2d(), left_distance, right_distance, starting_pose)
...
def test_simultaneous_vision_measurements():
    ...
    estimator.update_with_time(0.0, Rotation2d(), 0.0, 0.0)
...
def test_discard_stale_vision_measurements():
    ...
    t = 0.0
    while t < 4.0:
        estimator.update_with_time(t, Rotation2d(), 0.0, 0.0)
        t += 0.02
...
def test_sample_at():
    ...
    time = 1.0
    while time <= 2.0 + 1e-9:
        estimator.update_with_time(time, Rotation2d(), time, time)
        time += 0.02
    ...
    assert estimator.sample_at(1.01).x == pytest.approx(1.01, abs=1e-9)
...
def test_reset():
    ...
    estimator.reset_position(Rotation2d(), 1.0, 1.0, Pose2d(x=1, y=0, rotation=Rotation2d()))
