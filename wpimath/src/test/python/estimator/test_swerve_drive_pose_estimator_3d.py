...
_FL = Translation2d(x=1, y=1)
_FR = Translation2d(x=1, y=-1)
_BL = Translation2d(x=-1, y=-1)
_BR = Translation2d(x=-1, y=1)
...
def _make_positions():
    return [SwerveModulePosition() for _ in range(4)]
...
def _test_follow_trajectory(
    ...
    estimator.resetPosition(Rotation3d(), positions, Pose3d(starting_pose))
    ...
    while t < trajectory.totalTime():
        ...
        module_velocities = kinematics.toSwerveModuleVelocities(chassis_velocities)

        for i in range(4):
            distances[i] += module_velocities[i].velocity * dt
            positions[i] = SwerveModulePosition(
                distance=distances[i], angle=module_velocities[i].angle
            )
    ...
    if check_error:
        assert error_sum / num_steps < 0.058
        assert max_error < 0.2


def test_accuracy_facing_trajectory():
    kinematics = SwerveDrive4Kinematics(_FL, _FR, _BL, _BR)
    estimator = SwerveDrive4PoseEstimator3d(
        kinematics,
        Rotation3d(),
        _make_positions(),
        Pose3d(),
...
def test_simultaneous_vision_measurements():
    ...
    estimator.updateWithTime(0.0, Rotation3d(), _make_positions())
...
def test_discard_stale_vision_measurements():
    ...
    t = 0.0
    while t < 4.0:
        estimator.updateWithTime(t, Rotation3d(), _make_positions())
        t += 0.02
...
def test_sample_at():
    ...
    while time <= 2.0 + 1e-9:
        positions = [
            SwerveModulePosition(distance=time, angle=Rotation2d()) for _ in range(4)
        ]
        estimator.updateWithTime(time, Rotation3d(), positions)
        time += 0.02
...
def test_reset():
    ...
    pos1 = [SwerveModulePosition(distance=1.0, angle=Rotation2d()) for _ in range(4)]
    estimator.resetPosition(
        Rotation3d(), pos1, Pose3d(x=1, y=0, z=0, rotation=Rotation3d())
    )
    ...
    pos2 = [SwerveModulePosition(distance=2.0, angle=Rotation2d()) for _ in range(4)]
    estimator.update(Rotation3d(), pos2)
    ...
    pos3 = [SwerveModulePosition(distance=3.0, angle=Rotation2d()) for _ in range(4)]
    estimator.update(Rotation3d(), pos3)
