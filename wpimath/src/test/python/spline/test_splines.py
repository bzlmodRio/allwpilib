...
def _run_cubic(start, waypoints, end):
    start_cv, end_cv = SplineHelper.cubicControlVectorsFromWaypoints(
        start, waypoints, end
    )
    splines = SplineHelper.cubicSplinesFromControlVectors(start_cv, waypoints, end_cv)

    poses = []
    first_point = splines[0].getPoint(0.0)
...
def _run_quintic(start, end):
    spline = SplineHelper.quinticSplinesFromWaypoints([start, end])[0]
...
def test_cubic_s_curve():
    start = Pose2d(0, 0, Rotation2d.fromDegrees(90))
    waypoints = [Translation2d(1, 1), Translation2d(2, -1)]
    end = Pose2d(3, 0, Rotation2d.fromDegrees(90))
...
def test_cubic_throws_on_malformed():
    ...
    with pytest.raises(Exception):
        _run_cubic(
            Pose2d(10, 10, Rotation2d.fromDegrees(90)),
            [],
            Pose2d(10, 11, Rotation2d.fromDegrees(-90)),
        )
...
def test_quintic_squiggly_curve():
    _run_quintic(
        ...
        Pose2d(-1, 0, Rotation2d.fromDegrees(90)),
    )
