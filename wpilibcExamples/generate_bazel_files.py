...
def load_foldernames(filename):
    ...
    for test_data in data:
        ...
        output.append((test_data["foldername"], sorted(vendordeps), sorted(dynamic_deps)))
...
def dump_chunk(constant_name, chunk_list):
    ...
    for project_name, vendordeps, dynamic_deps in chunk_list:
        ...
        output += (
            f'    "{project_name}": ([{", ".join(f'"{x}"' for x in vendordeps)}], {dynamic_deps_txt}),\n'
        )
    output += "}\n"
    return output
...
def main():
    ...
    with open(output_file, "w", encoding="utf-8", newline="\n") as f:
        f.write('load("@bzlmodrio-opencv//libraries/cpp/opencv:libraries.bzl", "opencv_shared_libraries")\n\n')
