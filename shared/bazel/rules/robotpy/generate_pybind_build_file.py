...
class ResolveCastersConfig:
    def __init__(self, item: BuildTarget, include_root_prefix: str):
...
class PublishCastersConfig:
    def __init__(self, projectcfg, item: BuildTarget, include_root_prefix: str):
...
class BazelExtensionModule:
    def __init__(
        ...
        self.resolve_casters = ResolveCastersConfig(
            additional_extension_targets["resolve-casters"], include_root_prefix
        )
        ...
        for dep_name in all_dependencies:
            if "native" in dep_name:
                ...
                local_extension_dependencies.add(
                    f"//{fixup_root_package_name(base_library)}/src/main/python:{dep_name}"
                )
            else:
                raw_base_library = dep_name.split("_")[0]
                base_library = fixup_root_package_name(raw_base_library)
                local_extension_dependencies.add(
                    f"//{fixup_root_package_name(raw_base_library)}:{fixup_shared_lib_name(base_library)}"
                )
                dynamic_dependencies.add(
                    f"//{fixup_root_package_name(raw_base_library)}:shared/{fixup_shared_lib_name(base_library)}"
                )
                if dep_name != self.name:
                    local_extension_dependencies.add(
                        f"//{fixup_root_package_name(raw_base_library)}/src/main/python:{dep_name}_pybind_library"
                    )
...
def generate_pybind_build_file(
        ...
        if isinstance(item, ExtensionModule):
            extension_modules.append(
                BazelExtensionModule(
                    item, additional_extension_targets, include_root_prefix
                )
            )
            ...
            if item.command in [
                ...
                publish_casters_targets.append(
                    PublishCastersConfig(projectcfg, item, include_root_prefix)
                )
            else:
    ...
    def target_from_python_dep(python_dep):
        if "native" in python_dep:
            ...
            return f"//{fixup_root_package_name(base_library)}/src/main/python:{fixup_python_dep_name(python_dep)}"
